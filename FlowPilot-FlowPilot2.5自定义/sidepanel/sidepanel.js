...
function buildLogExportFileName(now = new Date()) {
  ...
  return `flowpilot-log-${timestamp}.txt`;
}

function formatLogEntryForExport(entry = {}) {
  const time = new Date(entry.timestamp || Date.now()).toLocaleString('zh-CN', {
    hour12: false,
    timeZone: DISPLAY_TIMEZONE,
  });
  const levelLabel = LOG_LEVEL_LABELS[entry.level] || entry.level || 'info';
  const step = Math.floor(Number(entry.step) || 0);
  const stepText = step > 0 ? ` 步${step}` : '';
  return `${time} ${levelLabel}${stepText} ${String(entry.message || '')}`.trim();
}

function buildLogExportContent(entries = visibleLogEntries) {
  const normalizedEntries = Array.isArray(entries) ? entries : [];
  if (normalizedEntries.length === 0) {
    return '暂无日志';
  }
  return normalizedEntries.map(formatLogEntryForExport).join('\n');
}

function exportCurrentLogEntries() {
  const content = buildLogExportContent(visibleLogEntries);
  downloadTextFile(content, buildLogExportFileName(), 'text/plain;charset=utf-8');
  showToast(`已导出 ${visibleLogEntries.length} 条日志`, 'success');
}
