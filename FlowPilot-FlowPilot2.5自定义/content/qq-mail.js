...
function buildMailCodeSearchText(item, sender = '', subject = '', digest = '') {
  const fullText = normalizeMailText(item?.textContent || '');
  const parts = [
    sender,
    subject,
    digest,
    fullText,
  ].map(normalizeMailText).filter(Boolean);
  return [...new Set(parts)].join(' ');
}

function isFreshQqMailListText(text = '') {
  const normalizedText = normalizeMailText(text).toLowerCase();
  if (!normalizedText) {
    return true;
  }
  if (/(\d+)\s*(?:小时|小時|hour|hours)\s*(?:前|ago)?/.test(normalizedText)) {
    return false;
  }
  if (/(?:昨天|昨日|yesterday)/.test(normalizedText)) {
    return false;
  }
  if (/\b\d{4}[-/.年]\d{1,2}[-/.月]\d{1,2}/.test(normalizedText)) {
    return false;
  }
  if (/\b\d{1,2}[-/.月]\d{1,2}(?:日)?\b/.test(normalizedText)) {
    return false;
  }
  return true;
}
...
async function handlePollEmail(step, payload) {
  ...
  const SINGLE_ATTEMPT_INITIAL_WAIT_MS = 20000;

  if (maxAttempts <= 1) {
    log(`步骤 ${step}：QQ 邮箱单次轮询，先等待 20 秒并刷新收件箱，避免读取旧验证码。`);
    await sleep(SINGLE_ATTEMPT_INITIAL_WAIT_MS);
    await refreshInbox();
    await sleep(800);
  }
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    const useFallback = attempt > FALLBACK_AFTER || maxAttempts <= 1;
    ...
    for (const item of allItems) {
      ...
      const searchText = buildMailCodeSearchText(item, sender, subject, digest);
      ...
      if (senderMatch || subjectMatch) {
        if (!isFreshQqMailListText(searchText)) {
          log(`步骤 ${step}：候选邮件时间过旧，已跳过（mailId：${mailId || 'unknown'}，主题：${subject.slice(0, 40)}，文本预览：${searchText.slice(0, 120)}）`, 'info');
          continue;
        }
        log(
          `步骤 ${step}：QQ 邮箱候选邮件命中（mailId：${mailId || 'unknown'}，发件人命中：${senderMatch ? '是' : '否'}，主题命中：${subjectMatch ? '是' : '否'}，主题：${subject.slice(0, 40)}，摘要长度：${String(digest || '').length}，正文长度：${searchText.length}）`,
          'info'
        );
        ...
        if (code) {
          ...
          log(`步骤 ${step}：已找到验证码：${code}（来源：${source}，mailId：${mailId || 'unknown'}，主题：${subject.slice(0, 40)}）`, 'ok');
          return { ok: true, code, emailTimestamp: Date.now(), mailId };
        }
        log(`步骤 ${step}：候选邮件未解析出验证码（mailId：${mailId || 'unknown'}，文本预览：${searchText.slice(0, 120)}）`, 'warn');
      }
    }
