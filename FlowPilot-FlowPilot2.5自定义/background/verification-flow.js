(function attachBackgroundVerificationFlow(root, factory) {
  ...
  const QQ_MAIL_SINGLE_ATTEMPT_RESPONSE_TIMEOUT_MS = 30000;
  ...
  function createVerificationFlowHelpers(deps = {}) {
    ...
    function resolveMailPollingTimeouts(mail, timedPoll) {
      ...
      const maxAttempts = Math.max(1, Math.floor(Number(payload?.maxAttempts) || 1));
      if (isQqMail(mail) && maxAttempts <= 1) {
        const responseTimeoutMs = Math.max(defaultResponseTimeoutMs, QQ_MAIL_SINGLE_ATTEMPT_RESPONSE_TIMEOUT_MS);
        return {
          payload,
          responseTimeoutMs,
          timeoutMs: Math.max(defaultTimeoutMs, responseTimeoutMs),
        };
      }
