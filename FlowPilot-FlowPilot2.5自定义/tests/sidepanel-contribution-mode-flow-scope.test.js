...
test('contribution mode manager does not project openai-only ui state into kiro flow', () => {
  ...
  assert.equal(dom.btnContributionMode.title, '贡献模式已禁用');
...
test('combined contribution tutorial button stays disabled and does not enable contribution mode', async () => {
  const windowObject = {};
  ...
  assert.equal(latestState.accountContributionEnabled, false);
  assert.equal(latestState.contributionAdapterId, '');
  assert.equal(latestState.activeFlowId, 'openai');
  assert.equal(latestState.flowId, 'openai');
  assert.equal(latestState.kiroTargetId, undefined);
  assert.equal(dom.btnContributionMode.disabled, true);
  assert.equal(dom.btnContributionMode.title, '贡献模式已禁用');
});
