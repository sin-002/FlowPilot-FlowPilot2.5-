...
test('verification flow keeps qq single-attempt polling timeout long enough for delayed scan', async () => {
  const mailPollCalls = [];
  ...
  await helpers.resolveVerificationStep(
    ...
    { provider: 'qq', source: 'qq-mail', label: 'QQ 邮箱' },
    {
      getRemainingTimeMs: async () => 5000,
      initialPollMaxAttempts: 1,
      resendIntervalMs: 0,
    }
  );
  ...
  assert.equal(mailPollCalls[0].options.responseTimeoutMs >= 30000, true);
  assert.equal(mailPollCalls[0].options.timeoutMs >= mailPollCalls[0].options.responseTimeoutMs, true);
});
